...
const revealTargets = Array.from(document.querySelectorAll(".case436-panel"));
...
const createIcon = (symbolId, className = "case-title-icon") => {
...
const caseTitleIcons = {
  "case436-toc": "icon-route",
  "case436-project": "icon-search-doc",
  "case436-framework": "icon-question",
  "case436-human-ai": "icon-columns",
  "case436-revision": "icon-pencil",
  "case436-reflection": "icon-lightbulb",
  "env361-context": "icon-book",
  "env361-problem": "icon-question",
  "env361-action": "icon-route",
  "env361-trust": "icon-check",
  "env361-reflection": "icon-compass",
  "env361-original": "icon-file-text",
  "env338-context": "icon-search-doc",
  "env338-conditions": "icon-map-pin",
  "env338-visuals": "icon-chart",
  "env338-compare": "icon-columns",
  "env338-cross": "icon-scale",
  "env338-insight": "icon-lightbulb",
  "env338-analysis": "icon-method",
  "env338-reflection": "icon-book",
  "env338-original": "icon-file-text",
  "env421-context": "icon-search-doc",
  "env421-approach": "icon-method",
  "env421-contribution": "icon-user",
  "env421-survey": "icon-chart",
  "env421-field": "icon-map-pin",
  "env421-synthesis": "icon-route",
  "env421-recommendations": "icon-compass",
  "env421-reflection": "icon-book",
  "env421-original": "icon-file-text",
};

Object.entries(caseTitleIcons).forEach(([id, symbolId]) => {
  const panel = document.getElementById(id);
  const label = panel?.querySelector(".case436-label, .case436-original");
  if (label && !label.querySelector(".case-title-icon")) {
    label.prepend(createIcon(symbolId));
  }
});

document.querySelectorAll(".case436-panel.case436-white").forEach((panel) => {
...
document
  .querySelectorAll(
    ".case436-toc-list a, .case436-framework-list div, .case436-text-row div, .case436-stack section, .case436-process-flow div, .case436-action-path section, .case436-compare section, .case338-session-grid figure, .case338-caption-row span, .case421-method-grid section, .case421-contribution-grid section, .case421-stat-grid div, .case421-synthesis section, .case421-recommendation-grid section, .case421-evidence-map section",
  )
  .forEach((element) => {
    element.classList.add("case421-interactive");
    element.tabIndex = 0;
    element.addEventListener("click", () => {
      element.parentElement
        ?.querySelectorAll(".case421-interactive")
        .forEach((sibling) => sibling.classList.remove("is-selected"));
    ...
    element.addEventListener("pointermove", (event) => {
      ...
      const tiltY = ((x / rect.width) - 0.5) * 1.2;
      const tiltX = ((0.5 - (y / rect.height)) * 1.2);
      element.style.setProperty("--mx", xPercent);
      element.style.setProperty("--my", yPercent);
